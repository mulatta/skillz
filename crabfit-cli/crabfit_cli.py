...
SPECIFIC_DATE_SLOT_LENGTH = 13
WEEKDAY_SLOT_LENGTH = 6
...
def cmd_respond(args: argparse.Namespace) -> None:
    ...
    event_times: list[str] = expand_time_slots(event["times"])
...
def expand_time_slots(slots: list[str]) -> list[str]:
    """Expand hourly crabfit event times to 15-minute availability slots."""
    expanded: list[str] = []
    for slot in slots:
        if slot[2:4] != "00":
        ...
        hour = slot[0:2]
        rest = slot[4:]
        expanded.extend(f"{hour}{minute}{rest}" for minute in ("00", "15", "30", "45"))
    return expanded
...
def parse_time_slot(
    ...
    Args:
        slot: Time slot in HHmm-DDMMYYYY or HHmm-d format.
        timezone: Target timezone for display.
        today: Reference UTC datetime for weekday-only slots.
    ...
    if len(slot) == SPECIFIC_DATE_SLOT_LENGTH:
        day = int(slot[5:7])
        month = int(slot[7:9])
        year = int(slot[9:13])
        utc_dt = dt.datetime(year, month, day, hour, minute, tzinfo=dt.UTC)
    elif len(slot) == WEEKDAY_SLOT_LENGTH:
        day_of_week = int(slot[5:6])
        utc_dt = _parse_weekday_time_slot(day_of_week, hour, minute, timezone, today)
    else:
        msg = f"Unsupported Crab.fit time slot format: {slot}"
        raise ValueError(msg)

    return utc_dt.astimezone(ZoneInfo(timezone))


def _parse_weekday_time_slot(
    day_of_week: int,
    hour: int,
    minute: int,
    timezone: str,
    ...
    """Parse HHmm-d slots using Crab.fit frontend's current-week anchoring."""
    reference = today or dt.datetime.now(tz=dt.UTC)
    reference_midnight = reference.astimezone(dt.UTC).replace(
        hour=0,
        minute=0,
        second=0,
        microsecond=0,
    ...
    local_reference = reference_midnight.astimezone(ZoneInfo(timezone))
    if local_dt.isocalendar().week > local_reference.isocalendar().week:
        utc_dt -= dt.timedelta(days=7)
...
def group_consecutive_slots(slots: list[str], timezone: str) -> list[list[str]]:
    ...
    for slot in slots:
        ...
        if curr_dt - prev_dt == dt.timedelta(minutes=15):
...
def cmd_show(args: argparse.Namespace) -> None:
    ...
    availability_map = build_availability_map(event_times, people)
    ...
    for count in sorted(slots_by_count.keys(), reverse=True):
        ...
        for group in group_consecutive_slots(slots, timezone):
